...
/**
 * Topographic contour field for the hero.
 * Sparse layer draws via GSAP (parent) or CSS fallback when animateOnMount.
 * Dense layer opacity tracks Timefold density.
 */
...
export function ContourBackground({
  ...
  density: number;
  /** When false, GSAP parent owns the draw-in (no CSS animation). */
  animateOnMount?: boolean;
}) {
  const denseOpacity = 0.12 + density * 0.5;
  ...
  return (
    ...
    >
      ...
      >
        {SPARSE.map((d, i) => (
          <path
            ...
            d={d}
            className={
              animateOnMount ? "contour-path draw-path" : "contour-path"
            }
            style={
              animateOnMount ? { animationDelay: `${i * 0.1}s` } : undefined
            }
          />
        ))}
      </svg>
      ...
      <svg
        className="absolute inset-0 h-full w-full transition-opacity duration-slow ease-out-soft"
